...
table_train = readtable('zip_train.csv');
num_train = table2array(table_train(:,2:end));
digit_train = table2array(table_train(:,1));
digit_num_train = str2double(digit_train);
data_train = [digit_num_train num_train];

table_test = readtable('zip_test.csv');
num_test = table2array(table_test(:,2:end));
digit_test = table2array(table_test(:,1));
digit_num_test = str2double(digit_test);
data_test = [digit_num_test num_test];

y_tr = data_train(:,1);
y_te = data_test(:,1);
X_tr = data_train(:,2:end);
X_te = data_test(:,2:end);
